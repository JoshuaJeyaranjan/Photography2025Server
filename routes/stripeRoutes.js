...
router.post("/create-checkout-session", async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (!dbSize) {
        return res.status(400).json({
          error: `Print size not found for ID: ${item.print_size_id}`,
        });
      }
    ...
    if (!req.body.shipping_rate) {
      return res
        .status(400)
        .json({ error: "Missing shipping rate selection." });
    }
...
router.get("/orders", authenticateJWT, checkAdmin, async (req, res) => {
  try {
    ...
    const orders = await db("orders")
      .select(
        // This is the correct and only query for base orders
        "id",
        "customer_name",
        "customer_email",
        "total_amount",
    ...
    if (event.type === "checkout.session.completed") {
      ...
      try {
        ...
        await req.transporter.sendMail({
          ...
          attachments: [
            { filename: `Invoice-${order.id}.pdf`, path: invoicePath },
          ],
        });
      } catch (err) {
